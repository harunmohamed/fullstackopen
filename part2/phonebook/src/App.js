...
const App = (props) => {
    ...
    const addEntry = (event) => {
        ...
        if (newNumber && newName) {
            checkPersonName.length ? alert(`${newName} is already in the phonebook`)
            : setPersons(persons.concat(entryObject));
        } else 
        alert('please comlete the name and the number field')
        setNewName('')
        setNewNumber('')
        console.log(persons)
    }
    ...
    const handleSearch = (event) => {
        ...
        if(inputText) { setPersons(results) } else { setPersons(props.persons) }
    }
